...
## Buffer and Dissolve 
Typical GIS operations are to create buffers around features (points, lines or polygons) and to dissolve polygon boundaries based on a common attribute - here we show how to do these common GIS tasks in an R workflow.

We'll demonstrate dissolving using both tidyverse functions in conjuntion with simple features as well as using the `st_union()` function in `sf`.

Let's use our Oregon cities and counties data again for this.

First we'll demonstrate buffering which is a simpler operation using `st_buffer` with a suplied distance.

First we pick our largest cities and buffer them by some arbitrary distance - we'll say 20 miles (note below we convert fee to miles).
```{r}
library(sf)
library(dplyr)
library(mapview)
