...
### Faceting and We can revisit the Multnomah County Census American Community Survey (ACS) data we used in `ggplot2` example above with `tmap` to map college education and income for Multnomah County, Oregon:
```{r}
library(tmap)
